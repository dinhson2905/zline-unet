...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        assert len(mask_file) == 1, \
            f'Either no mask or multiple masks for the ID {idx}: {mask_file}'
        assert len(img_file) == 1, \
            f'Either no image or multiple images found for the ID {idx}: {mask_file}'
...
class StomachDataset(BasicDataset):
